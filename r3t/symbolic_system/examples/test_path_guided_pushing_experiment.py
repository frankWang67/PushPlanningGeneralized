...
def distance_between_poses(start, end):
    """
    ...
    if len(start) == 2 and len(end) == 2:
        ...
        weights = [1, 1, np.linalg.norm([0.07, 0.12], ord=2)/2]
...
def guided_sampler():
    ...
    round_try = 0
    while True:
        sample = np.random.uniform(search_space_dimensions[:, 0], search_space_dimensions[:, 1])
        path_point_dim = guide_path.shape[1]
        if path_point_dim == 2:
            distance_array = np.linalg.norm(guide_path-sample[0:2], axis=1)
        else:
            distance_list = [distance_between_poses(path_point, sample) for path_point in guide_path]
            distance_array = np.array(distance_list)
        nearest_idx = np.argmin(distance_array)
        acceptance = np.exp(-0.5*((distance_array[nearest_idx]**2)/(0.1**2)))
        if np.random.uniform() <= acceptance or round_try >= max_try:
            return sample
        round_try += 1
